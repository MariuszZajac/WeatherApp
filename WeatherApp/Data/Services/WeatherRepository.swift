...
final class WeatherRepository: WeatherRepositoryProtocol {
  ...
  func fetchWeatherData(latitude: Double, longitude: Double) async throws -> WeatherData {
    do {
      if weatherDataCache.isCacheFresh(name: cashFileName),
        let data = weatherDataCache.fetchWeatherData(fileName: cashFileName)
      {
        return data
      } else {
        let response = try await weatherAPIService.downloadWeatherData(
